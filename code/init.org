...
#+begin_src emacs-lisp
  ...
  (setq langtool-language-tool-jar "~/biblemacs/resources/LanguageTool-6.6/languagetool-commandline.jar")
...
(add-to-list 'load-path (expand-file-name "~/biblemacs/lisp/"))
...
(require 'read-aloud)
(require 'sermon-index)
(require 'sermon-create)
#+end_src
...
:END:
#+begin_src emacs-lisp
  (load-file "~/biblemacs/lisp/whisper/whisper.el")
...
#+begin_src emacs-lisp
    ...
    (define-key evil-normal-state-map (kbd "m") 'evil-org-forward-sentence)
    (define-key evil-normal-state-map (kbd "M") 'evil-org-backward-sentence))
...
After navigating, recenter so the found text is at the top of the buffer."
  (interactive)
  (let* ((bible-directory "~/biblemacs/bible/")
...
#+begin_src emacs-lisp
    ...
    (defun bm/init    ()(interactive) (find-file "~/biblemacs/init.org"))

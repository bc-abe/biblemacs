;;; sword-to-org.el --- Convert Sword modules to Org outlines -*- lexical-binding: t -*-
...
(defun sword-to-html-layout (module key)
...
(defun sword-to-org-book (module key)
  "Insert Org outline like a book."
  (interactive (list (if (or current-prefix-arg
                             (not sword-to-org-default-module))
                         (completing-read "Module: " (sword-to-org--diatheke-get-modules))
                       sword-to-org-default-module)
...
(defun sword-insert-ref (module key)
  "Insert Org outline in current buffer for Sword MODULE and KEY.
The buffer will be switched to `text-mode' before inserting, to
improve performance, and then switched back to `org-mode' if
it was active."
...
(defun sword-insert-html (module key)
  "Insert Org outline in current buffer for Sword MODULE and KEY.
The buffer will be switched to `text-mode' before inserting, to
improve performance, and then switched back to `org-mode' if
it was active."
  ...
  (let ((was-org-mode (eq major-mode 'org-mode)))
    ...
    (cl-loop with last-book
             ...
             do (-let (((&plist :book book :chapter chapter :verse verse :text text) passage))
                  (unless (equal book last-book)
                    (insert (format "<p class=\"verse\"><b>%s</b> <br>\n" key))
                    (setq last-chapter nil)
                    (setq last-book book))
                  (unless (equal chapter last-chapter)
                    (insert (format "" chapter))
                    (setq last-chapter chapter))
                  (insert (format "<b>%s</b> %s<br> \n" verse text ))
                  ))
	     (insert (format"</p>"))
    (when was-org-mode
      (org-mode))))
...
(defun sword-ref-text (module key)
  "Insert Org outline in current buffer for Sword MODULE and KEY.
The buffer will be switched to `text-mode' before inserting, to
improve performance, and then switched back to `org-mode' if
it was active."
  ...
  (let ((was-org-mode (eq major-mode 'org-mode)))
    ...
    (cl-loop with last-book
             ...
             for passage in (sword-to-org--diatheke-parse-text
                             (sword-to-org--diatheke-get-text module key))
             do (-let (((&plist  :verse verse :text text) passage))
                  (insert (format "*%s* %s \n" verse text ))))
	     (insert (format""))
    (when was-org-mode
      (org-mode))))

(defun sword-ref-from-region ()
  "Delete highlighted text and use it as the key for `sword-to-org-insert-ref`."
  (interactive)
  (if (use-region-p)
      (let* ((key (string-trim (buffer-substring-no-properties (region-beginning) (region-end))))
             (module (if (or current-prefix-arg
                             (not sword-to-org-default-module))
                         (completing-read "Module: " (sword-to-org--diatheke-get-modules))
                       sword-to-org-default-module)))
        ;; Delete the region
        (delete-region (region-beginning) (region-end))
        ;; Insert passage using the key
        (sword-insert-ref module key))
    (message "No region selected.")))

(defun sword-multiple-refs-from-region ()
  "Delete highlighted text (one Bible reference per line) and insert passages for each using `sword-to-org-insert-ref`."
  (interactive)
  (if (use-region-p)
      (let* ((region-text (buffer-substring-no-properties (region-beginning) (region-end)))
             (references (split-string region-text "\n" t "[[:space:]]*"))
             (module (if (or current-prefix-arg
                             (not sword-to-org-default-module))
                         (completing-read "Module: " (sword-to-org--diatheke-get-modules))
                       sword-to-org-default-module)))
        ...
        (dolist (ref references)
          (let ((trimmed (string-trim ref)))
            (when (not (string-empty-p trimmed))
              (sword-insert-ref module trimmed)))))
    (message "No region selected.")))

(defun sword-multiple-refs-with-links ()
  "Replace highlighted Bible references with linked headings and insert each passage.
Each reference becomes an Org heading like '*[[file:...][Book Chapter:Verse]]*', followed by the verse text from `sword-to-org-insert-ref`."
  (interactive)
  (if (use-region-p)
      (let* ((region-start (region-beginning))
             ...
             (module (if (or current-prefix-arg
                         ...
                         (completing-read "Module: " (sword-to-org--diatheke-get-modules))
                       sword-to-org-default-module))
             (bible-dir "~/studio/site/bible/"))
        (delete-region region-start region-end)
        (dolist (ref references)
          (let* ((trimmed (string-trim ref)))
            (when (not (string-empty-p trimmed))
              (let* ((parts (split-string trimmed " +" t)) ; e.g., ("Ezekiel" "44:9")
                     (book (capitalize (car parts)))
                     (verse (mapconcat #'identity (cdr parts) ""))
                     (file (concat book ".org"))
                     (link (format "*[[file:%s%s::%s][%s %s]]*" bible-dir file verse book verse)))
                (insert link "\n")
                (sword-ref-text module trimmed)
                (insert "\n"))))))
    (message "No region selected.")))



(defun sword-to-notes (module key)
  "Insert Org outline in current buffer for Sword MODULE and KEY.
The buffer will be switched to `text-mode' before inserting, to
improve performance, and then switched back to `org-mode' if
it was active."
    ...
    (cl-loop with last-book
             ...
             do (-let (((&plist :book book :chapter chapter :verse verse :text text) passage))
                  ...
                  (unless (equal chapter last-chapter)
                    ...
                    (setq last-chapter chapter))
                  (insert (format "  *%s* %s\n" verse text ))))
	     (insert (format"  #+end_verse\n"))
    (when was-org-mode
      (org-mode))))

(defun sword-to-org-insert-verse-blocks-from-region ()
  "Delete highlighted references and insert each as a blank level-2 Org heading with a verse block."
  (interactive)
  (if (use-region-p)
      (let* ((region-text (buffer-substring-no-properties (region-beginning) (region-end)))
             (references (split-string region-text "\n" t "[[:space:]]*"))
        ...
        (delete-region (region-beginning) (region-end))
        (dolist (ref references)
          (let* ((key (string-trim ref))
                 (passages (sword-to-org--diatheke-parse-text
                            (sword-to-org--diatheke-get-text module key))))
            (insert "** \n#+begin_verse\n")
            (insert (format "*%s* \n" key))
            (cl-loop for passage in passages
                     do (pcase-let ((`(:book ,_ :chapter ,_ :verse ,verse :text ,text) passage))
                          (insert (format "*%s* %s \n" verse text))))
            (insert "#+end_verse\n\n"))))
    (message "No region selected.")))
